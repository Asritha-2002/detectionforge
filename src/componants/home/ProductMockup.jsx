  ...
  return (
    <div className="overflow-hidden rounded-xl border border-border bg-surface shadow-2xl shadow-black/30 ring-1 ring-black/5">
      ...
      <div className="grid grid-cols-12 gap-px bg-border">
        ...
        {/* Mobile tab switcher — the sidebar above is desktop-only */}
        <div className="col-span-12 flex gap-1 overflow-x-auto border-b border-border bg-surface px-3 py-2 md:hidden">
          {[
            ["detections", "Detections"],
            ["validation", "Validation"],
            ["coverage", "Coverage"],
            ...
            <button
              key={key}
              onClick={() => setActiveTab(key)}
              className={`shrink-0 whitespace-nowrap rounded-md px-2.5 py-1 text-xs ${
                activeTab === key
                  ? "bg-accent text-foreground"
                  : "text-muted-foreground"
              }`}
    ...
    <>
      <div className="col-span-12 bg-background px-4 py-4 md:col-span-7">
          <div className="mb-4 flex items-center justify-between">
            ...
            <button className="inline-flex h-7 shrink-0 items-center gap-1.5 rounded-md border border-teal/40 bg-teal/10 px-2.5 text-xs font-medium text-teal">
              <Play className="h-3 w-3" /> Run replay
            </button>
          </div>

          {/* Horizontal scroll wrapper: the table below has fixed-width
              columns that don't fit narrow mobile screens. Rather than
              squeezing/clipping the columns, this lets the table keep
              its full desktop layout and scroll sideways on mobile. */}
        ...
        <div className="mt-4 overflow-x-auto rounded-lg border border-border bg-surface">
          <div className="min-w-[360px]">
            ...
            {[
              ...
              <div
                key={index}
                className="flex items-center justify-between gap-3 border-b border-border/50 px-4 py-3 text-xs last:border-b-0"
              >
                <div>
                ...
                <span className="shrink-0 whitespace-nowrap rounded-full border border-teal/30 bg-teal/10 px-2 py-1 text-[10px] text-teal">
                  {item.status}
                </span>
              </div>
            ))}
